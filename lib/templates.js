...
// Simpan gambar (base64, dari lib/gemini.js#generateTemplateImageWithAI) ke
// Google Drive lewat Apps Script, lalu tautkan URL-nya ke template terkait.
// Dipisah dari addTemplate karena templateId baru diketahui SETELAH template
// teksnya tersimpan (nama file gambarnya pakai templateId itu).
export async function saveTemplateAsset(templateId, { mimeType, base64 }) {
  const url = process.env.GOOGLE_SHEETS_WEBHOOK_URL;
  const secret = process.env.APPS_SCRIPT_SECRET;
  if (!url) throw new Error("GOOGLE_SHEETS_WEBHOOK_URL belum diset.");
  if (!secret) throw new Error("APPS_SCRIPT_SECRET belum diset.");
  ...
  const res = await fetch(url, {
    method: "POST",
    headers: { "Content-Type": "text/plain" },
    body: JSON.stringify({ action: "saveTemplateAsset", templateId, mimeType, dataBase64: base64, secret }),
  });
  const data = await res.json();
  if (data.ok === false) throw new Error(data.error ?? "Gagal menyimpan gambar.");
